...
- **Named state** (v4.34): read with `this.state.n` **or** `this.state["n"]` — both work, the
  translator rewrites the dot form. Write with `this.setState("n", v)`; assigning to the dict would
  write to a copy, since the instance is ephemeral. Keyed by **name instead of call order**, so
  unlike `useState` these are legal inside an `if` or a loop.
